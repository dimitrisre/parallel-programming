...
package object barneshut {
  ...
  case class Fork(
    ...
    def insert(b: Body): Fork = {
      if(distance(nw.centerX-size/2, nw.centerY-size/2, b.x, b.y) <= nw.size)
        Fork(nw.insert(b), ne, sw, se)
      else if(distance(ne.centerX-size/2, ne.centerY-size/2, b.x, b.y) <= ne.size)
        Fork(nw, ne.insert(b), sw, se)
      else if(distance(sw.centerX-size/2, sw.centerY-size/2, b.x, b.y) <= sw.size)
  ...
  extends Quad {
    ...
    val (massX, massY) = (
      bodies.aggregate(0f)(
        (sum, b) => sum + (b.mass*b.x),
        (sum1, sum2) => sum1 + sum2
      )/mass,
      bodies.aggregate(0f)(
        (sum, b) => sum + (b.mass*b.y),
        (sum1, sum2) => sum1 + sum2
      )/mass )
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      val x: Int = (
          (b.x - boundaries.minX)/(boundaries.width / sectorPrecision)
        ).toInt
      val y: Int = (
          (b.y - boundaries.minY)/(boundaries.height / sectorPrecision)
        ).toInt
      this.apply(x, y) += b
      this
    }
    ...
    def combine(that: SectorMatrix): SectorMatrix = {
      for (i <- matrix.indices)
        this.matrix.update(i,
          this.matrix(i).combine(that.matrix(i))
        )
      this
    }
