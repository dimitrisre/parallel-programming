    ...
    if(radius == 0) src(x,y)
      ...
      while(xi<=clamp(x+radius, 0, src.width-1)) {
        ...
        while(yi<=clamp(y+radius, 0, src.height-1)) {
          rm = rm + red(src(xi, yi))
          gm = gm + green(src(xi, yi))
          bm = bm + blue(src(xi, yi))
          am = am + alpha(src(xi, yi))
      ...
      rgba(rm/pixelsCount, gm/pixelsCount, bm/pixelsCount, am/pixelsCount)
    }
  }
