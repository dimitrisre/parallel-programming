...
object LineOfSight {
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float], startingAngle: Float, from: Int, until: Int): Unit = {
    ...
    @tailrec
    def loop(start: Int, end: Int, max:Float): Unit = {
      if(start < end){
        output(start) = List(input(start)/start, max).max
        loop(start+1, end, output(start))
      }
    }

    loop(from, until, startingAngle)
  }
