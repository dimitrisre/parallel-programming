...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val clusters = points.par.groupBy(findClosest(_,means))
    means.par.map(
      m => m -> clusters.getOrElse(m,GenSeq())
    ).toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.par.map(
      m => findAverage(m, classified.get(m).get)
    )
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    val res = for(i <- 0 until oldMeans.length) yield (oldMeans(i), newMeans(i))

    res.par.aggregate(true)(
      (conv, p)=>conv && (p._1.squareDistance(p._2) <= eta),
      _ && _
    )
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    ...
    else // your implementation need to be tail recursive
